...
#include "cvxgen/solver.h"
  ...
  /* dim is used to offset the result in the output vecor, according to which dimension
   * (x,y,z) is being calculated */
  /* x - 0 */
  /* y - 1 */
  /* z - 2 */
  /* yaw - 0 */
    ...
    int dimension)
  {
    ...
    if (tempQ.size() == 4) {
      for (int i = 0; i < 4; i++) {
        if (tempQ[i] >= 0 && std::isfinite(tempQ[i])) {
          ...
          ROS_ERROR_STREAM("CvxWrapper - Q matrix has to be PSD - parameter "
                           << i << " !!! Safe value of 500 set instead");
          myQ[i] = 500;
        }
      }
    } else {
      ROS_ERROR_STREAM("CvxWrapper - Q matrix wrong size "
                       << tempQ.size() << " !!! Safe values set instead");
      myQ[0] = 5000;
      myQ[1] = 0;
      myQ[2] = 0;
      myQ[3] = 0;
    ...
    if (dt <= 0 || !std::isfinite(dt)) {
      ROS_ERROR_STREAM("CvxWrapper - dt parameter wrong "
                       << dt << " !!! Safe value of 0.01 set instead");
      dt = 0.01;
    }

    if (dt2 <= 0 || !std::isfinite(dt2)) {
      ROS_ERROR_STREAM("CvxWrapper - dt2 parameter wrong "
                       << dt2 << " !!! Safe value of 0.2 set instead");
      dt2 = 0.2;
    }
  ...
  bool CvxWrapper::setVelQ(double Q_vel)
  {
    if (Q_vel < 0.0) {
      ROS_ERROR_STREAM(
        "[MpcTracker]: CvxWrapper - Q vel has to be positive!! Q_vel" << Q_vel << " !!!");
      return false;
    }
    myQ[1] = Q_vel;
    return true;
  ...
  bool CvxWrapper::setQ(std::vector<double> Qnew)
  {
    ...
    if (Qnew.size() == 4) {
      for (int i = 0; i < 4; i++) {
        if (Qnew[i] >= 0 && std::isfinite(Qnew[i])) {
          ...
          ROS_ERROR_STREAM(
            "[MpcTracker]: CvxWrapper - Q matrix has to be PSD - parameter " << i
                                                                             << " !!!");
          result = false;
        }
      }
    ...
    if (result) { ROS_INFO("[MpcTracker]: CvxWrapper - successfully set matrix Q"); }
    return result;
  }
  ...
  {
    for (int i = 0; i < horizon_len; i++) { *params.x_ss[i + 1] = reference(i, 0); }
  }
  ...
  {
    for (int i = 0; i < 4; i++) { params.Q[i] = myQ[i]; }
    return solve();
  }
